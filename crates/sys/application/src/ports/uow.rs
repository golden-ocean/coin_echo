...
use crate::ports::{DictionaryItemRepository, DictionaryRepository};
...
impl ErrorMeta for UnitOfWorkError {
    ...
    fn code(&self) -> &'static str {
        match self {
            Self::Connection => "sys.uow.connection_failed",
            Self::Commit => "sys.uow.commit_failed",
            Self::Rollback => "sys.uow.rollback_failed",
            Self::Begin => "sys.uow.begin_failed",
            Self::TransactionClosed => "sys.uow.transaction_closed",
        }
    }
}
